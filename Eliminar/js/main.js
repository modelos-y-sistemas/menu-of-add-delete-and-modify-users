    ...
    $.ajax({
        ...
        success: function(data){
            ...
            for(var i=0;i<resp.length;i++){
                tabla+='<tr class="list-users__tr"><td class="list-users__td">'+resp[i].NombreDelUsuario+
                '</td><td class="list-users__td">'+resp[i].Mail+'</td><td class="list-users__td">'+resp[i].Codigo_Curso+
                '</td><td class="list-users__td">'+resp[i].ID+
                '</td><td class="list-users__td"> <input type="checkbox" name="user'
                +resp[i].ID+'" id="'+resp[i].ID+'"> </td></tr>';
            }
